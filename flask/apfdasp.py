    ...
    def define_routes(self):
        ...
        def register():
            ...
            if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'phone' in request.form and 'address' in request.form and 'uuid' in request.form:
                ...
                # print("user exiting", existing_user)
                if existing_user:
                    ...
                    # print("fahdg.eapfhaisugtfaksugfuhkasgfukaw",current_time)
                ...
                msg = 'Please fill out the form!asdfghjk'
            return render_template('register.html', msg=msg)



        @self.app.route('/users_accounts', methods=['GET', 'POST'])
        def users_accounts():
            if 'username' in session:
                conn = self.get_db_connection()
                cursor = conn.cursor(dictionary=True)
                cursor.execute(f"SELECT * FROM {self.table_name}")
                customers = cursor.fetchall()  # Lấy tất cả dữ liệu 
                cursor.close()
                conn.close()
